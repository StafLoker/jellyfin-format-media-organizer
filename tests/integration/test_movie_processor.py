...
def test_all_tokens(films_dir, source_file, mock_tmdb):
...
def test_tmdb_provides_year(films_dir, source_file, mock_tmdb):
...
def test_no_tmdb_match(films_dir, source_file):
...
def test_no_quality(films_dir, source_file, mock_tmdb):
    ...
    dest = films_dir / "Inception (2010) [tmdbid-27205].mkv"
    assert dest.exists()


def test_dest_already_exists_overwritten(films_dir, source_file, mock_tmdb):
...
def test_dry_run(films_dir, source_file, mock_tmdb):
    config.DRY_RUN = True
    src = source_file("Inception.2010.1080p.mkv")
    ctx = _ctx(str(src), title="Inception", year="2010", quality="[1080p]")
...
def test_source_missing(films_dir, mock_tmdb):  # noqa: ARG001
